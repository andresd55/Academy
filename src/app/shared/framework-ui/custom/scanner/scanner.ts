...
import { Footer, Header, PrimeNGConfig, PrimeTemplate, SharedModule } from '../../primeng/api/public_api';
import { UniqueComponentId } from '../../primeng/utils/uniquecomponentid';
import { DomHandler } from '../../primeng/dom/public_api';
import { ZIndexUtils } from '../../primeng/utils/public_api';
import { FocusTrapModule } from '../../primeng/focustrap/public_api';
import { RippleModule } from '../../primeng/ripple/public_api';
...
@Component({
    ...
    template: `
        <div *ngIf="maskVisible" [class]="maskStyleClass"
            [ngClass]="{'p-scanner-mask': true, 'p-component-overlay p-component-overlay-enter': this.modal, 'p-scanner-mask-scrollblocker': this.modal || this.blockScroll,
            ...
            <div #container [ngClass]="{'p-scanner p-component':true, 'p-scanner-rtl':rtl,'p-scanner-draggable':draggable,'p-scanner-resizable':resizable, 'p-scanner-maximized': maximized}"
                [ngStyle]="style" [class]="styleClass" *ngIf="visible" pFocusTrap [pFocusTrapDisabled]="focusTrap === false"
                [@animation]="{value: 'visible', params: {transform: transformOptions, transition: transitionOptions}}" (@animation.start)="onAnimationStart($event)" (@animation.done)="onAnimationEnd($event)" role="scanner" [attr.aria-labelledby]="id + '-label'">
                <div #titlebar class="p-scanner-header" (mousedown)="initDrag($event)" *ngIf="showHeader">
                    <span [attr.id]="id + '-label'" class="p-scanner-title ttile-page f-18" *ngIf="!headerFacet && !headerTemplate">{{header}}</span>
                    <span [attr.id]="id + '-label'" class="p-scanner-title" *ngIf="headerFacet">
                        <ng-content select="p-header"></ng-content>
                    </span>
                    <ng-container *ngTemplateOutlet="headerTemplate"></ng-container>
                    <div class="p-scanner-header-icons">
                        <button *ngIf="maximizable" type="button" [ngClass]="{'p-scanner-header-icon p-scanner-header-maximize p-link':true}" (click)="maximize()" (keydown.enter)="maximize()" tabindex="-1" pRipple>
                            <span class="p-scanner-header-maximize-icon" [ngClass]="maximized ? minimizeIcon : maximizeIcon"></span>
                        </button>
                        <button *ngIf="closable" class="close" type="button" [ngClass]="{'p-scanner-header-icon p-scanner-header-close p-link':true}" [attr.aria-label]="closeAriaLabel" (click)="close($event)" (keydown.enter)="close($event)" [attr.tabindex]="closeTabindex" pRipple>
    ...
    styleUrls: ['../scanner/scanner.css'],
    host: {
        'class': 'p-element'
    }
})
export class scanner implements AfterContentInit, OnInit, OnDestroy {
    ...
    initDrag(event: MouseEvent) {
        if (DomHandler.hasClass(event.target, 'p-scanner-header-icon') || DomHandler.hasClass((<HTMLElement>event.target).parentElement, 'p-scanner-header-icon')) {
            return;
        }
