...
@Component({
  ...
  styleUrls: ['../viewer/viewer.component.css'],
  host: {
      'class': 'p-element'
  }
})
export class Viewer implements AfterContentInit, OnInit, OnDestroy {
  ...
  initDrag(event: MouseEvent) {
      if (DomHandler.hasClass(event.target, 'p-viewer-header-icon') || DomHandler.hasClass((<HTMLElement>event.target).parentElement, 'p-viewer-header-icon')) {
          return;
      }
  ...
  fullImage(zoomOut = false) {
      const imageViewer = $('.image-viewer')[0];
      const image = $('.image-viewer img')[0];
      const content = $('.p-viewer-content')[0];
      const controls = $('.constrols-viewer')[0];
      
      this.full = !this.full;
      if(this.full) {
        $(content).css('margin-top', '0px');
        $(controls).css('margin-top', '160px');
        $(imageViewer).css('width', '100%');
        $(image).css('height', (screen.height - 184) + 'px');
      } else {
        $(content).css('margin-top', '60px');
        $(controls).css('margin-top', '100px');
        if(!zoomOut) {
            $(imageViewer).css('width', '405px');
        }
  ...
  zoomIn() {
    const imageViewer = $('.image-viewer')[0];
    const newWidth = $(imageViewer).width() + ($(imageViewer).width() * 0.1);
    $(imageViewer).css('width', newWidth + 'px');
  }
  ...
  turnLeft() {
    const image = $('.image-viewer img')[0];
    this.angle -= 90;
    $(image).css('transform','rotate(' + this.angle + 'deg)');
  }
...
@NgModule({
  imports: [CommonModule, FocusTrapModule, RippleModule, TooltipModule, TranslateModule.forRoot({
    loader: {
  ...
  exports: [Viewer, SharedModule, TooltipModule],
  declarations: [Viewer]
})
